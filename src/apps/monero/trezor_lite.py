...
class TrezorLite(object):
    ...
    def __init__(self):
        ...
        self.iface = trezor_iface.get_iface()
        self.debug = True
        self.purge = False
    ...
    async def tsx_init(self, tsx_data: MoneroTsxData):
        ...
        try:
            tsxd = await trezor_misc.tsx_data_translate(tsx_data)
...
class TTransaction(object):
    ...
    async def tsx_inputs_done(self):
        ...
        if self.in_memory():
            await self.tsx_inputs_done_inm()
    ...
    async def final_msg(self):
        ...
        return MoneroTsxFinalResp(cout_key=cout_key, salt=salt, rand_mult=rand_mult, tx_enc_keys=b''.join(list(tx_enc_keys)))
