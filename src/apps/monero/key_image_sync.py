...
async def layout_key_image_sync(state, ctx, msg):
    log.debug(__name__, '### KI SYNC. Free: {} Allocated: {}'.format(gc.mem_free(), gc.mem_alloc()))
    log.debug(__name__, 'KI sync state: %s', state.ctx_ki)

    from apps.monero.protocol import key_image_sync
    log.debug(__name__, '### KI sync importedd. Free: {} Allocated: {}'.format(gc.mem_free(), gc.mem_alloc()))
    ...
    try:
        if msg.init:
            ...
            state.ctx_ki = key_image_sync.KeyImageSync(ctx=ctx, iface=iface.get_iface(ctx))
